...
function teardownRealtime() {
  if (Array.isArray(realtimeChannels) && realtimeChannels.length && supabaseRealtime) {
    realtimeChannels.forEach(channel => {
      try { supabaseRealtime.removeChannel(channel); } catch {}
    });
  }
  realtimeChannels = [];
}
...
async function setupRealtime() {
  try {
    ...
    const publicChannel = supabaseRealtime
      .channel(`rt-public-${loggedUser}-${Date.now()}`)
      .on("postgres_changes", { event: "*", schema: "public", table: "rt_messages" }, () => {
        debounceRealtimeRefresh(() => loadMessages());
      })
      .on("postgres_changes", { event: "*", schema: "public", table: "rt_online_users" }, () => {
        debounceRealtimeRefresh(() => { loadTypingStatus(); loadOnlineUsers(); }, 80);
      })
      .on("postgres_changes", { event: "*", schema: "public", table: "rt_private_channels" }, () => {
        if (chatMode === "dm") debounceRealtimeRefresh(() => loadMessages(), 80);
      })
      .on("postgres_changes", { event: "*", schema: "public", table: "rt_private_messages" }, () => {
        if (chatMode === "dm") debounceRealtimeRefresh(() => loadMessages(), 80);
      })
      .subscribe((status) => {
        realtimeReady = status === "SUBSCRIBED";
        pollingEnabled = !realtimeReady;
      });
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  messagesIntervalId = setInterval(() => {
    if (pollingEnabled) loadMessages();
  }, 3000);

  updateOnlineStatus();
  onlineIntervalId = setInterval(() => {
    updateOnlineStatus();
    if (pollingEnabled) loadOnlineUsers();
  }, 5000);
  ...
  typingIntervalId = setInterval(() => {
    if (pollingEnabled) loadTypingStatus();
  }, 900);
});
