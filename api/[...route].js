...
const RT_USERS_TABLE = "rt_users";
const RT_USER_PROFILES_TABLE = "rt_user_profiles";
...
async function handleRegister(req, res) {
  ...
  try {
    ...
    if (!isValidName(username)) {
      return sendJson(res, 400, { success: false, message: "Usuário inválido. Use 2 a 24 caracteres: letras, números ou _." });
    }

    if (password.length < 3 || password.length > 100) {
      return sendJson(res, 400, { success: false, message: "Senha inválida. Use pelo menos 3 caracteres." });
    }

    const safeDisplayName = (display_name || username).slice(0, 40);
    ...
    if (existing) {
      return sendJson(res, 409, { success: false, message: "Esse usuário já existe." });
    }
    ...
    if (insertUserError) {
      return sendJson(res, 500, { success: false, message: insertUserError.message });
    }

    await supabaseService
      .from(RT_USER_PROFILES_TABLE)
      .upsert([{
        username,
        display_name: safeDisplayName,
        avatar_url: null,
    ...
    return sendJson(res, 200, { success: true, message: "Conta criada com sucesso." });
  } catch (err) {
...
async function handleRealtimeConfig(req, res) {
  ...
  return sendJson(res, 200, {
    ...
    tables: {
      messages: PUBLIC_MESSAGES_TABLE,
...
async function handler(req, res) {
  ...
  if (routeKey === "register") return handleRegister(req, res);
